...
export const booking=async(req,res)=>{
    ...
    let order=new Orders({userId,OrderedBy,ProductId,productName,Quantity,price,bill,category});
    ...
    (eventInfo.Bookedproducts =
      eventInfo.Bookedproducts + NoOfItems),
...
export const getallorders=async(req,res)=>{
  ...
  try{
    ...
    let alll =await Orders.aggregate( [{$project:{"productName":1,"Quantity":1,"bill":1,"OrderplacedDate":1}}]);
...
export const updatingbooking = async (req, res) => {
  ...
  try {
    ...
    if (!book || book.userId.toString() !== req.user.id) {
      return res
        .status(StatusCodes.GONE)
        .json({ error: "id invalid or id not found in the orders collection while updating the orders" });
    }
